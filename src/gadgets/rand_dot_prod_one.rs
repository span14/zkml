...
type RandDotProductOneConfig = GadgetConfig;

pub struct RandDotProductOneChip<F: PrimeField> {
  config: Rc<RandDotProductOneConfig>,
  _marker: PhantomData<F>,
}

impl<F: PrimeField> RandDotProductOneChip<F> {
  pub fn construct(config: Rc<RandDotProductOneConfig>) -> Self {
    Self {
      config,
      _marker: PhantomData,
    }
  ...
  pub fn configure(meta: &mut ConstraintSystem<F>, gadget_config: GadgetConfig) -> GadgetConfig {
    ...
    meta.create_gate("rand dot product one gate", |meta| {
      let s = meta.query_selector(selector);
      let gate_inp = RandDotProductOneChip::<F>::get_input_columns(&gadget_config)
        .iter()
        .map(|col| meta.query_advice(*col, Rotation::cur()))
        .collect::<Vec<_>>();
    
      let gate_output = meta.query_advice(columns[columns.len() - 1], Rotation::cur());
      let c = &gadget_config.challenges[0].expr();
    ...
    selectors.insert(GadgetType::RandDotProductOne, vec![selector]);
  ...
  fn op_row_region_rand(
    ...
    if self.config.use_selectors {
      let selector = self.config.selectors.get(&GadgetType::RandDotProductOne).unwrap()[0];
      selector.enable(region, row_offset).unwrap();
    }

    let inp_cols = RandDotProductOneChip::<F>::get_input_columns(&self.config);
    inp
      .iter()
      .enumerate()
...
impl<F: PrimeField> Gadget<F> for RandDotProductOneChip<F> {
  fn name(&self) -> String {
  ...
  fn forward(
    ...
    let c = layouter.get_challenge(self.config.challenges[0]);
    ...
    Ok(
      adder_chip
        .forward(
          layouter.namespace(|| "rand dot product one adder"),
          &vec![tmp],
          single_inputs,
        )
        .unwrap(),
